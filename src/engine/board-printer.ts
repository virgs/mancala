...
export class BoardPrinter {
    ...
    public print(pockets: BoardConfig): void {
        ...
        console.log([topPlayerPockets[topPlayerPockets.length - 1], bottomPlayerPockets[bottomPlayerPockets.length - 1]]
            .map((stones, index) => this.printHole(stones, pockets.length - (index * pockets.length / 2) - 1))
            .join(Array.from(Array(topPlayerInternalPocket.length)).map(() => this.emptySpaceGap).join(this.emptySpaceGap)))

        console.log(this.emptySpaceGap + bottomPlayerInternalPocket
