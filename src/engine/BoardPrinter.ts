...
export class BoardPrinter {
  ...
  public print(pockets: BoardConfig): void {
    const topPlayerPockets = pockets.filter(
      (_pocket, index) => index < pockets.length / 2,
    );
    const topPlayerInternalPocket = topPlayerPockets.filter(
      (_pocket, index) => index < topPlayerPockets.length - 1,
    );
    const bottomPlayerPockets = pockets.filter(
      (_pocket, index) => index >= pockets.length / 2,
    );
    ...
    console.log(
      this.emptySpaceGap +
        topPlayerInternalPocket
          .reverse()
          .map((stones, index) =>
            this.printHole(stones, pockets.length - index - 2),
          )
    ...
    console.log(
      [
        ...
        .map((stones, index) =>
          this.printHole(
            stones,
            pockets.length - (index * pockets.length) / 2 - 1,
          ),
        )
        .join(
